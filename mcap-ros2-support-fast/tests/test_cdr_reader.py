...
from mcap_ros2_support_fast._cdr import CdrReader, CdrWriter, EncapsulationKind
...
rcl_interfaces__parameter_event = (
...
def test_parse_tfmessage() -> None:
    data = bytes.fromhex(tf2_msg__tf_message)
...
def test_parse_parameter_event() -> None:
    data = bytes.fromhex(rcl_interfaces__parameter_event)
    reader = CdrReader(data)
    ...
    assert reader.sequence_length() == 0
    assert reader.uint8_array(0) == b""  # bool[] bool_array_value
    assert reader.sequence_length() == 0
...
def test_arrays() -> None:
    ...
    for [write_fn, read_fn, array, output] in (
        ...
        assert getattr(reader, read_fn)(len(array)) == (output or array)
        assert reader.decoded_bytes() == len(data)
