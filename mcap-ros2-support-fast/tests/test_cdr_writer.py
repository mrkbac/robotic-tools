...
from mcap_ros2_support_fast._cdr import CdrReader, CdrWriter
...
def test_serialize() -> None:
    ...
    assert buffer.getvalue() == bytes.fromhex(tf2_msg_tf_essage)
...
def test_alignment() -> None:
    ...
    assert bytes.hex(buffer.getvalue()) == "000100000100000000000000"
    writer.write_uint8(2)
    assert bytes.hex(buffer.getvalue()) == "00010000010000000000000002"
    writer.write_uint16(3)
    assert bytes.hex(buffer.getvalue()) == "00010000010000000000000002000300"
    writer.write_uint32(4)
    assert bytes.hex(buffer.getvalue()) == "0001000001000000000000000200030004000000"
