...
def sessions_cmd(
    ...
    *,
    ...
    sql = (
        ...
        "       (SELECT COUNT(*) FROM file_observation obs "
        "        WHERE obs.scan_session_id = s.id) AS observations, "
        "       (SELECT COUNT(*) FROM content c "
        "        WHERE c.first_seen_scan_session_id = s.id) AS new_content, "
        "       (SELECT COUNT(*) FROM scan_error se "
        "        WHERE se.scan_session_id = s.id) AS errors "
        "FROM scan_session s "
