...
def test_resolve_mcap_path_ros2_bag_directory(tmp_path: Path) -> None:
    bag = tmp_path / "mybag"
    bag.mkdir()
...
def test_resolve_mcap_path_directory_without_matching_file_unchanged(tmp_path: Path) -> None:
    bag = tmp_path / "mybag"
    bag.mkdir()
    (bag / "other.mcap").write_bytes(b"\x89MCAP0\r\n")

    assert resolve_mcap_path(str(bag)) == str(bag)
