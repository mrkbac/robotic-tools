...
import pytest
from small_mcap import ChannelNotFoundError, include_topics, read_message
from small_mcap.reader import breakup_chunk
from small_mcap.records import Channel, Chunk, Message, MessageIndex, Schema
...
class TestChunkSkipping:
    ...
    def ten_messages_file(self) -> Path:
        ...
        path = CONFORMANCE_DATA / "TenMessages" / "TenMessages-ch.mcap"
        assert path.is_file(), f"Required conformance fixture is missing: {path}"
        return path

    @pytest.fixture
    def ten_messages_with_index_file(self) -> Path:
        """Path to test file with 10 messages in chunks WITH MessageIndex."""
        path = CONFORMANCE_DATA / "TenMessages" / "TenMessages-ch-mx.mcap"
        assert path.is_file(), f"Required conformance fixture is missing: {path}"
        return path
    ...
    def one_message_file(self) -> Path:
        ...
        path = CONFORMANCE_DATA / "OneMessage" / "OneMessage-ch.mcap"
        assert path.is_file(), f"Required conformance fixture is missing: {path}"
        return path
    ...
    def test_index_optimization_preserves_definition_order_errors(self, is_seekable: bool):
        chunk_data = io.BytesIO()
        Message(1, 0, 10, 10, b"x").write_record_to(chunk_data)
        Schema(1, "Test", "json", b"{}").write_record_to(chunk_data)
        Channel(1, 1, "/test", "json", {}).write_record_to(chunk_data)
        chunk = Chunk(10, 10, len(chunk_data.getvalue()), 0, "", chunk_data.getvalue())

        output = io.BytesIO()
        writer = McapWriter(output)
        writer.start()
        writer.add_chunk(chunk, {1: MessageIndex(1, [10], [0])})
        writer.finish()
    ...
    def test_time_filter_skips_all_chunks(self, ten_messages_file: Path):
        ...
        with patch("small_mcap.reader.breakup_chunk", wraps=breakup_chunk) as mock_breakup:
            ...
            # The terminal chunk cannot provide definitions to any later record.
    ...
    def test_time_filter_skips_chunks_before_range(self, ten_messages_file: Path):
        ...
        assert [message.log_time for _schema, _channel, message in messages] == [5, 7, 8, 9]
    ...
    def test_time_filter_skips_chunks_after_range(self, ten_messages_file: Path):
        ...
        assert sorted(message.log_time for _schema, _channel, message in messages) == [
            0,
            1,
            2,
            3,
...
class TestChannelDefinitionPreservation:
    ...
    def test_preserves_channel_definitions_from_skipped_chunks(self):
        ...
        """
        ten_messages_file = CONFORMANCE_DATA / "TenMessages" / "TenMessages-ch.mcap"
        assert ten_messages_file.is_file(), (
            f"Required conformance fixture is missing: {ten_messages_file}"
        )
